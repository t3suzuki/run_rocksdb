...
def get_cmd(op, n_th, cache_capacity, workload, dbname, count):
    common_args = "-db rocksdb -P workloads/{} -P rocksdb/rocksdb.properties -s -p rocksdb.cache_size={} -p rocksdb.dbname={} -p threadcount={}".format(workload, cache_capacity, dbname, n_th)
    if op == "set":
        cmd = "./ycsb -load -p recordcount={} {}".format(count, common_args)
    else:
        cmd = "./ycsb -run -p recordcount={} {}".format(count, common_args)
    return cmd
...
def run(mode, op, n_core, n_th, cache_capacity, workload, count):
    ...
    drive_ids = ["0000:0d:00.0","0000:0e:00.0"]
    ...
    subprocess.run("make compact -B N_TH={} DB_PATH={}".format(n_th, db_path).split())
    if mode == "abt":
    ...
    subprocess.run("make N_TH={} DB_PATH={} USE_SCHED_YIELD={} SET_DATA={} CACHE_CAPACITY={}".format(n_th, db_path, use_sched_yield, set_data, cache_capacity).split())
...
run("native", "get", 10, 10, 1*1024*1024*1024, "workloada", 1000*1000)
